      ...
      <div class="card-content">
        <router-link :to="myLink">
          ...
          <p class="is-size-5 has-text-dark">
            {{ String(numberOfPartic) }} Participants
          </p>
          <p class="is-size-7 has-text-primary">
            {{ new Date(dates[0]).toUTCString() }}
          </p>
          <p class="is-size-7 has-text-primary">
            {{ new Date(dates[1]).toUTCString() }}
          </p>
        </router-link>
      </div>
...
export default {
  name: "ContestPreview",
  props: ["title", "dates", "link", "numberOfPartic"],
  data() {
    return {
      showDelete: false,
  ...
  methods: {
    removeContest() {
      this.$buefy.dialog.confirm({
        ...
        onConfirm: () =>
          contestService.removeContest(this.link).then(() => {
            this.$router.go();
          }),
      });
    },
